...
def run_pipeline(ref_img_path, ref_ann_path, sub_img_path, result_path, suffix):
    ...
    print(f'files conversion took {time.time() - start_time} s')
    step_time = time.time()
    ...
    print(f'reference segmentation took {time.time() - step_time} s')
    step_time = time.time()
    ...
    print(f'preprocess subject took {time.time() - step_time} s')
    step_time = time.time()
    ...
    print(f'subject segmentation took {time.time() - step_time} s')
    step_time = time.time()
    ...
    print(f'point cloud and deformation took {time.time() - step_time} s')
    step_time = time.time()
    ...
    # using ants
    if not os.path.isfile(join(sub_save_path, 'ants', 'ccf_affined', name + '_warped_ccf_annotation' + suffix)):
        print(f'using ants to register sample: {sub_img_path.split("/")[-1]}')

        record_path = join(sub_save_path, 'ants')
        ...
        # aff = np.loadtxt(join(sub_save_path, 'affines', name + '_sub2ccf_wo.txt'))
        # tx = ants.create_ants_transform(dimension=3, matrix=aff)
        # ants.write_transform(tx, join(sub_save_path, 'affines', name + '_sub2ccf_wo.mat'))
        #
        # aff = np.loadtxt(join(sub_save_path, 'affines', name + '_ccf2sub_wo.txt'))
        # tx = ants.create_ants_transform(dimension=3, matrix=aff)
        # ants.write_transform(tx, join(sub_save_path, 'affines', name + '_ccf2sub_wo.mat'))
